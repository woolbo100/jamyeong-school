      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        ...
        <div className="mx-auto mt-10 max-w-3xl rounded-[28px] border border-amber-300/20 bg-white/5 p-8 text-center text-white">
          ...
          <p className="mt-3 text-white/70">
            자명스쿨은 AI를 배우는 곳이 아니라 마음의 흐름을 현실의
            구조로 바꾸는 학교입니다.
          </p>
          <Link
            href="/courses"
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {relatedPosts.map((item) => (
                <Link
                  key={item.slug}
                  href={`/blog/${item.slug}`}
                  className="rounded-[24px] border border-white/10 bg-white/5 p-5 text-white transition hover:border-amber-300/30 hover:bg-white/10"
                >
